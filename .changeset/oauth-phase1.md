...
- Provider-backed OAuth now links the site to the shared provider `app.yaml` as soon as BYO app credentials are saved, so `auth status` / `reset` can see that staged shared app during dry-runs or failed browser flows without hiding any still-active site-local token until the shared OAuth token actually exists.
